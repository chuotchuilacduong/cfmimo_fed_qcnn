...
def generate_dataset(dir_path, num_clients, niid, balance, partition):
    ...
    # Lấy dữ liệu từ trainloader
    for data, target in trainloader:
        dataset_image.append(data)
        dataset_label.append(target)

    # Lấy dữ liệu từ testloader
    for data, target in testloader:
        dataset_image.append(data)
        dataset_label.append(target)

# Chuyển từ list tensor sang numpy
    dataset_image = torch.cat(dataset_image, dim=0).cpu().detach().numpy()
    dataset_label = torch.cat(dataset_label, dim=0).cpu().detach().numpy()
